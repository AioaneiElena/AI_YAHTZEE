...
class YahtzeeTraining:
    ...
    def calculate_reward(self, action, dice_roll):
        ...
        if action == 0:  # Ones
            ...
            if any(count >= 3 for count in counts):  # Verificăm dacă există un număr care apare de cel puțin 3 ori
                # Adunăm doar valorile care se repetă de cel puțin 3 ori
                for value, count in enumerate(counts, start=1):
                    if count >= 3:
                        reward = value * 3  # Adunăm valoarea numărului respectiv, de 3 ori
                        break  # După ce găsim primul număr care se repetă de 3 ori, ieșim din buclă

        elif action == 7:  # Four-of-a-kind
            if any(count >= 4 for count in counts):  # Verificăm dacă există un număr care apare de cel puțin 4 ori
                # Adunăm doar valorile care se repetă de cel puțin 4 ori
                for value, count in enumerate(counts, start=1):
                    if count >= 4:
                        reward = value * 4  # Adunăm valoarea numărului respectiv, de 4 ori
